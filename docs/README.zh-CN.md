...
- **上下文长度按模型解析** —— 目录里有这个模型就用它自己的上下文窗口和输出上限（取各来源里最小的那个），没有就退回路由上的 `providerContextWindow` / `providerMaxTokens`。
...
上下文长度只从模型目录来（`/v1/models` 里根本没有这个字段）：目录描述了这个 id 就用它的上下文窗口和输出上限，多个来源不一致时取**最小**的那个；目录里没有就退回路由上的 `providerContextWindow` / `providerMaxTokens`，并且插件会在日志里点名哪些模型用的是猜测值。
...
| 图片输入与上下文长度解析 | ✅ | 按插件行 → 控制台标签 → 模型目录的顺序判定，容量取最小 |
...
不冲突，是同一份数据：插件写入的就是网页版「模型」页写的那两个地方（`llm-pi-ai` 设置和凭据服务）。区别只在所有权 —— 网关提供的那个路由的模型列表由网关决定，插件每次启动和每次「同步模型」都会整体替换它。所以想改模型参数，请改在网关侧或者改插件行，不要改在 DSH 侧。
...
会。那个路由的模型列表由网关决定，每次启动和每次「同步模型」都会整体替换。想要不同的数值，就改插件行的 `providerContextWindow` / `providerMaxTokens`，或者依赖模型目录给出的真实值。
